...
let nodejs = {
    ...
    closure: {
        do {
            ...
            let user_input = (input "Enter version of node.js (see https://github.com/nodesource/distributions#ubuntu-versions): ")
            $"deb [signed-by=/etc/apt/keyrings/nodesource.gpg] https://deb.nodesource.com/node_($user_input).x nodistro main" | sudo tee /etc/apt/sources.list.d/nodesource.list
            sudo apt update
